...
describe('TasksController (e2e)', () => {
  ...
  it('(POST /tasks) should return a validation error when sending invalid task data ', async () => {
    ...
    const { body } = await request(app.getHttpServer())
      .post('/tasks').send(taskDto).expect(400);
  ...
  it('(POST /tasks) should create a new task and get it back', async () => {
    ...
    const { body: createTask } = await request(app.getHttpServer())
      .post('/tasks').send(taskDto).expect(201);
    ...
    const { body: getTask } = await request(app.getHttpServer())
      .get(`/tasks/${createTask.id}`).send().expect(200);
...
it('/tasks/:id (PATCH) should update a task by ID', async () => {
  ...
  const findTask = (await tasksService.findLots())[2];
